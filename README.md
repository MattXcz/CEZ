...
**Hodinová dodávka (`pnd/data`) pro výrobní/mikrozdrojové EAN zatím nefunguje** – koordinátor pro ně používá stejný `assemblyCode` jako pro spotřebu (`05`), který MEPAS gateway u výrobního EAN odmítá s HTTP 400. Živě ověřeno (issue #24): gateway nabízí pro dodávku samostatné, sudé kódy (`02`, `04`, `06`, `08`, `10`, `12`) jako protějšek lichých kódů u spotřeby (`01`, `03`, `05`, `07`, `09`, `11`) na stejném partnerovi – seznam pro váš účet zjistíte přes `scripts/test_pnd_consumption.py --status`. Který z nich vrací přímo hodinovou energii v kWh (obdoba `05` u spotřeby) místo výkonu v kW po 15 minutách (obdoba `03`), ještě nevíme jistě – zjišťujeme.
...
Poznámka k issue #24: pole `"ammAktivni"` v `get_supply_points` **není** podmínkou hodinových dat – u odběrného místa s `ammAktivni: false`, ale `typMereni: "B"` (průběhové měření) a `potencialniPND: true` import hodinové spotřeby funguje normálně. Dřívější verze tohoto README tvrdila opak; bylo to mylné. Pro výrobní/mikrozdrojové EAN (`typ` `V`/`M`) zatím `pnd/data` s naším `assemblyCode` vrací HTTP 400 – který kód gateway pro dodávku nabízí, ukáže `scripts/test_pnd_consumption.py --status` (viz komentáře ve skriptu); pokud ho máte, přiložte výstup do issue.
