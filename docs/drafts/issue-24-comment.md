...
**Proč tam ten request vůbec byl.** Integrace se přihlašovala v opačném pořadí než prohlížeč: šla rovnou na `/cas/login?service=…` a `authorize` volala až po přihlášení. Prohlížeč (tlačítko „Přihlásit" na portálu) začíná právě tím `authorize` requestem a login je jeho důsledek. Ve **v2.0.3** je `login()` přeuspořádaný do pořadí prohlížeče - jeden `authorize` na začátku, žádný opakovaný request po loginu, a ten řádek z logu zmizel úplně.
